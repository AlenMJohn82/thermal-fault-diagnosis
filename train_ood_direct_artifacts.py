...
def main():
    ...
    # Strategy 2: "Direct Artifacts" - train on clean data WITH dynamic artifacts from epoch 1, for 50 epochs.
    print("\n" + "="*60)
    print("STRATEGY: DIRECT ARTIFACTS (NO STAGE 1 OR 2)")
    print("="*60)
    
    # apply_seen_ood = True means YES artifacts
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4) # Fixed LR across all epochs
    ...
    torch.save(model.state_dict(), "thermal_model_direct_art_ood_trained.pth")
    print("\n✓ Direct-Artifact Model saved to thermal_model_direct_art_ood_trained.pth")
