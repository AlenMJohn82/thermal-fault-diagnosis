...
# We use torchvision.models for the baseline ResNet18
...
class BaselineResNet(nn.Module):
    """Standard ResNet18 Baseline Architecture (No Physics Guidance)"""
    def __init__(self, num_classes=11):
        super().__init__()
        self.backbone = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
        # Modify final FC layer for our 11 classes
...
class OODThermalDatasetBaseline(ThermalDataset):
    """Dataset wrapper for baseline model (doesn't need masks or phys feats returned)"""
    ...
    def __getitem__(self, idx):
        ...
        # Apply artifact to the RAW image before conversion
        if self.apply_seen_ood:
            severity = random.randint(1, 5)
            if random.random() < 0.5:
                # Apply artifact using our artifact library
...
def train_one_epoch_baseline(model, loader, optimizer, criterion, device):
    model.train()
    total_loss = 0
    # Note: Baseline loader only returns (img_tensor, label)
    for imgs, labels in loader:
        imgs, labels = imgs.to(device), labels.to(device)
        optimizer.zero_grad()
        outputs = model(imgs) # Baseline model takes only image
...
# A basic dataset class for the baseline when there are no dynamic artifacts
class BaselineThermalDataset(torch.utils.data.Dataset):
...
def main():
    ...
    # Load the exact same split used for PG-CNN
    ...
    # Need to match the labels logically with the paths. Alternatively, just reload and re-split safely since random_state=42
    path_clean = os.path.join(base_path, "IR-Motor-bmp")
    clean_paths, clean_labels = load_dataset_paths(path_clean, CLASS_MAP)
    train_clean_p_check, _, train_clean_l_check, _ = train_test_split(
        clean_paths, clean_labels, test_size=0.30, stratify=clean_labels, random_state=42
    )
    
    # Ensure they match
    assert train_clean_p == train_clean_p_check, "Split paths mismatch!"
    train_clean_l = train_clean_l_check
    
    train_basenames = {os.path.basename(p) for p in train_clean_p}
    print(f"Loaded FIXED untouched split (Train:{len(train_clean_p)})")

    print("\n" + "="*60)
    print("STEP 2: FILTER AUGMENTED & CREATE DATASETS (BASELINE)")
    print("="*60)
    ...
    # Baseline loader for dynamic seen artifacts
    train_loader_clean = DataLoader(
        OODThermalDatasetBaseline(train_clean_p, train_clean_l, img_size=224, apply_seen_ood=True), 
        batch_size=32, shuffle=True, num_workers=2
    )
    
    sep_loader = DataLoader(BaselineThermalDataset(sep_p, sep_l, img_size=224), batch_size=32, shuffle=True, num_workers=2)
    stoch_loader = DataLoader(BaselineThermalDataset(stoch_p, stoch_l, img_size=224), batch_size=32, shuffle=True, num_workers=2)

    model = BaselineResNet(num_classes=11).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4) # Same learning rate
    criterion = nn.CrossEntropyLoss()

    epochs_1, epochs_2, epochs_3 = 20, 20, 10 # Same curriculum stages

    print("\n" + "="*60)
    print("STAGE 1: SEPARATE PHYSICS AUG")
    print("="*60)
    for ep in range(epochs_1):
        loss = train_one_epoch_baseline(model, sep_loader, optimizer, criterion, device)
    ...
    for ep in range(epochs_2):
        loss = train_one_epoch_baseline(model, stoch_loader, optimizer, criterion, device)
    ...
    for ep in range(epochs_3):
        loss = train_one_epoch_baseline(model, train_loader_clean, optimizer, criterion, device)
        print(f"  Epoch {ep+1}/{epochs_3} | Loss: {loss:.4f}")

    torch.save(model.state_dict(), "thermal_model_baseline_ood_trained.pth")
    print("\n✓ Baseline OOD-Robust Model saved to thermal_model_baseline_ood_trained.pth")
