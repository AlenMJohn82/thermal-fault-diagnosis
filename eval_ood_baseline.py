...
# Use the ResNet baseline definition exactly as in training script
import torchvision.models as models
from torchvision.models import ResNet18_Weights
from train_ood_baseline import BaselineResNet, BaselineThermalDataset
...
class EvaluatorOODDatasetBaseline(BaselineThermalDataset):
    """Dataset wrapper to apply specific artifacts at a specific severity for the baseline."""
...
def run_evaluation_baseline(model, device, test_paths, test_labels, artifact_func=None, severity=1):
    dataset = EvaluatorOODDatasetBaseline(test_paths, test_labels, 224, artifact_func, severity)
    ...
    with torch.no_grad():
        for imgs, lbs in loader:
            imgs = imgs.to(device)
            out = model(imgs) # Baseline only needs images
...
def main():
    ...
    # 2. Load trained baseline model
    model = BaselineResNet(num_classes=11).to(device)
    model.load_state_dict(torch.load("thermal_model_baseline_ood_trained.pth", map_location=device))
    model.eval()
    ...
    print("="*60)
    acc_clean, _, _ = run_evaluation_baseline(model, device, test_paths, test_labels, None, 1)
    ...
    for s in seen_severities:
        acc, _, _ = run_evaluation_baseline(model, device, test_paths, test_labels, apply_seen_artifacts, s)
    ...
    for s in unseen_severities:
        acc, _, _ = run_evaluation_baseline(model, device, test_paths, test_labels, apply_unseen_artifacts, s)
    ...
    for name, func in unseen_types:
        type_accs = []
        for s in unseen_severities:
            acc, _, _ = run_evaluation_baseline(model, device, test_paths, test_labels, func, s)
    ...
    with open("ood_results_summary_baseline.json", "w") as f:
        json.dump(res_dict, f, indent=2)

    print("\n✓ Baseline Metrics saved to ood_results_summary_baseline.json")
