...
from train_ood_baseline_mask import BaselineMaskResNet, BaselineMaskThermalDataset
...
class EvaluatorOODDatasetBaselineMask(BaselineMaskThermalDataset):
    """Dataset wrapper to apply specific artifacts at a specific severity for the baseline+mask."""
    ...
    def __getitem__(self, idx):
        ...
        # Ensure mask is calculated on the corresponding noisy/clean image
...
def run_evaluation_baseline_mask(model, device, test_paths, test_labels, artifact_func=None, severity=1):
    dataset = EvaluatorOODDatasetBaselineMask(test_paths, test_labels, 224, artifact_func, severity)
    loader = DataLoader(dataset, batch_size=32, shuffle=False, num_workers=2)
    
    all_preds, all_true = [], []
    with torch.no_grad():
        for imgs, masks, lbs in loader:
...
def main():
    ...
    model.load_state_dict(torch.load("thermal_model_baseline_mask_ood_trained.pth", map_location=device))
    model.eval()
    ...
    print("="*60)
    acc_clean, _, _ = run_evaluation_baseline_mask(model, device, test_paths, test_labels, None, 1)
    ...
    for s in seen_severities:
        acc, _, _ = run_evaluation_baseline_mask(model, device, test_paths, test_labels, apply_seen_artifacts, s)
    ...
    for s in unseen_severities:
        acc, _, _ = run_evaluation_baseline_mask(model, device, test_paths, test_labels, apply_unseen_artifacts, s)
    ...
    for name, func in unseen_types:
        type_accs = []
        for s in unseen_severities:
            acc, _, _ = run_evaluation_baseline_mask(model, device, test_paths, test_labels, func, s)
    ...
    with open("ood_results_summary_baseline_mask.json", "w") as f:
        json.dump(res_dict, f, indent=2)

    print("\n✓ Baseline+Mask Metrics saved to ood_results_summary_baseline_mask.json")
