...
def main():
    ...
    try:
        ...
        print("Waiting for Baseline+Physics results to complete. Run eval_ood_baseline_phys.py first.")
        return
    ...
    plt.plot(severities, mask_seen, color='orange', linestyle='-', marker='^', label='Ablation 1: Baseline + Mask Only', linewidth=2)
    plt.plot(severities, phys_seen, color='purple', linestyle='--', marker='D', label='Ablation 2: Baseline + Phys Only', linewidth=2)
    ...
    plt.plot(severities, mask_unseen, color='orange', linestyle='-', marker='^', label='Ablation 1: Baseline + Mask Only', linewidth=2)
    plt.plot(severities, phys_unseen, color='purple', linestyle='--', marker='D', label='Ablation 2: Baseline + Phys Only', linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('ood_ablation_final_comparison.png', dpi=300)
    print("✓ Saved 4-way final ablation plot to ood_ablation_final_comparison.png")
    
    # Print numerical comparison
    print("\n--- Final Ablation Summary Comparison ---")
    print(f"Mean Seen   | PG-CNN: {np.mean(pg_seen)*100:.2f}% | Base+Mask: {np.mean(mask_seen)*100:.2f}% | Base+Phys: {np.mean(phys_seen)*100:.2f}% | Baseline: {np.mean(base_seen)*100:.2f}%")
    print(f"Mean Unseen | PG-CNN: {np.mean(pg_unseen)*100:.2f}% | Base+Mask: {np.mean(mask_unseen)*100:.2f}% | Base+Phys: {np.mean(phys_unseen)*100:.2f}% | Baseline: {np.mean(base_unseen)*100:.2f}%")
