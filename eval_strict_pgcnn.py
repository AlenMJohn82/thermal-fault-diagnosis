...
def main():
    ...
    # Load from the Strict pipeline dataset
    test_paths, test_labels = load_dataset_paths("Strict_OOD_Pipeline/TEST/Clean", CLASS_MAP)

    print("\n" + "="*60)
    print(f"EVALUATING STRICT PIPELINE MODEL (TEST SET: {len(test_paths)})")
    print("="*60)
    
    evaluate_model(
        "thermal_model_strict_pipeline.pth", 
        test_paths, test_labels, device, 
        "ood_results_summary_strict_pipeline.json"
    )
    print("\n✓ Strict Pipeline Evaluation Complete!")
