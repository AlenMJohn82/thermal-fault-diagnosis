...
class BaselinePhysResNet(nn.Module):
    """Baseline ResNet18 + concatenation of 4 Physics features (No Hotspot Masking, No adaptive fusion)"""
    def __init__(self, num_classes=11, num_phys_features=4):
        super().__init__()
        backbone = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
        self.backbone = nn.Sequential(*list(backbone.children())[:-1]) # up to pool
        
        # Fc layer takes 512 (visual) + 4 (physics)
        self.fc = nn.Linear(512 + num_phys_features, num_classes)

    def forward(self, img, phys_feat):
        # Extract visual features -> [B, 512, 1, 1]
        visual_feats = self.backbone(img)
        visual_feats = visual_feats.view(visual_feats.size(0), -1) # [B, 512]
        
        # Concatenate physics features -> [B, 516]
        fused_feats = torch.cat((visual_feats, phys_feat), dim=1)
...
class OODThermalDatasetBaselinePhys(ThermalDataset):
    """Dataset wrapper for baseline+physics model (Needs phys_tensor returned)"""
    ...
    def __getitem__(self, idx):
        ...
        # Generate mask purely to extract physics features (the mask itself isn't returned)
        ...
        return img_tensor, phys_tensor, label
...
def train_one_epoch_phys(model, loader, optimizer, criterion, device):
    model.train()
    total_loss = 0
...
class BaselinePhysThermalDataset(ThermalDataset):
...
def main():
    ...
    print("\n" + "="*60)
    print("STEP 2: FILTER AUGMENTED & CREATE DATASETS (BASELINE + PHYSICS)")
    print("="*60)
    ...
    train_loader_clean = DataLoader(
        OODThermalDatasetBaselinePhys(train_clean_p, train_clean_l, img_size=224, apply_seen_ood=True), 
        batch_size=32, shuffle=True, num_workers=2
    )
    
    sep_loader = DataLoader(BaselinePhysThermalDataset(sep_p, sep_l, img_size=224), batch_size=32, shuffle=True, num_workers=2)
    stoch_loader = DataLoader(BaselinePhysThermalDataset(stoch_p, stoch_l, img_size=224), batch_size=32, shuffle=True, num_workers=2)

    model = BaselinePhysResNet(num_classes=11).to(device)
    ...
    for ep in range(epochs_1):
        loss = train_one_epoch_phys(model, sep_loader, optimizer, criterion, device)
    ...
    for ep in range(epochs_2):
        loss = train_one_epoch_phys(model, stoch_loader, optimizer, criterion, device)
    ...
    for ep in range(epochs_3):
        loss = train_one_epoch_phys(model, train_loader_clean, optimizer, criterion, device)
        print(f"  Epoch {ep+1}/{epochs_3} | Loss: {loss:.4f}")

    torch.save(model.state_dict(), "thermal_model_baseline_phys_ood_trained.pth")
    print("\n✓ Baseline+Physics OOD Model saved to thermal_model_baseline_phys_ood_trained.pth")
