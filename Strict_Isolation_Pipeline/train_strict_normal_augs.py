...
def main():
    ...
    print("\n" + "="*60)
    print("STEP 1: LOADING ISOLATED 'NORMAL AUGMENTATION' DATASETS")
    print("="*60)
    ...
    # 2. Load Normal Separate Augmentations
    physics_paths, physics_labels = load_dataset_paths(os.path.join(base_dir, "Normal_Aug_Separate"), CLASS_MAP)
    print(f"Loaded Normal Separate Augs: {len(physics_paths)} images")

    # 3. Load Normal Combined Augmentations
    stoch_paths, stoch_labels = load_dataset_paths(os.path.join(base_dir, "Normal_Aug_Combined"), CLASS_MAP)
    print(f"Loaded Normal Combined Augs: {len(stoch_paths)} images")

    print("\n" + "="*60)
    print("STEP 2: CONFIGURING DATALOADERS")
    print("="*60)

    # Stage 1 loader (Standard Separate)
    sep_loader = DataLoader(ThermalDataset(physics_paths, physics_labels, img_size=224), batch_size=32, shuffle=True, num_workers=2)
    # Stage 2 loader (Standard Combined)
    stoch_loader = DataLoader(ThermalDataset(stoch_paths, stoch_labels, img_size=224), batch_size=32, shuffle=True, num_workers=2)
    # Stage 3 loader (Original Clean images, No Augmentation as requested)
    ...
    print("\n" + "="*60)
    print("STAGE 1: (STANDARD) SEPARATE AUGMENTATIONS")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("STAGE 2: (STANDARD) COMBINED AUGMENTATIONS")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("STAGE 3: ORIGINAL (NO AUGMENTATION)")
    print("="*60)
    ...
    torch.save(model.state_dict(), "thermal_model_normal_augs.pth")
    print("\n✓ Normal Augs Pipeline Model saved to thermal_model_normal_augs.pth")
